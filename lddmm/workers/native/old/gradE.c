...
void gradE(double *_w, double t, double *yt, double *rhot, int L, int R, int dim, int CSP, int CSD, double *scales2, double *scaleweight2, int GL, int GS, int GI)
{
    double w = 0;
    int CSPL = CSP*L;
    int CSDL = CSD*L;
    ...
    for (i=0; i<L; i++) { /* particle */
        ...
        for (l=0; l<L; l++) { /* particle */
            ...
            for (s=0; s<R; s++) { /* scale */
                ...
                /* debug */
                /*mexPrintf("dximdxl %f %f\n",dximdxl[0],dximdxl[1]);
                mexPrintf("d1kXximxl %f %f\n",d1kXximxl[0],d1kXximxl[1]);
                mexPrintf("vt1 %f %f\n",vt1[0],vt1[1]);*/
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if(nrhs!=11) {
        mexErrMsgIdAndTxt("gradC:nrhs","11 inputs required.");
    }
    if(nlhs!=1) {
    ...
    GS = (double)(mxGetScalar(prhs[9]))-1;
    GI = (double)(mxGetScalar(prhs[10]))-1;
    ...
    gradE(w,t,yt,rhot,L,R,dim,CSP,CSD,scales2,scaleweight2,GL,GS,GI);
}
